...
/*   Updated: 2021/03/27 11:06:05 by amalliar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			*monitor_start(void *arg)
{
	...
	while (1)
	{
		min_ttd = sim_data->time_to_die;
		i = 0;
		while (i < sim_data->num_philos)
		{
			ttd = sim_data->time_to_die - get_timestamp() + \
				philo_stat_tab[i].last_time_eaten;
			if (ttd <= 0 && sim_data->sim_is_running)
				return (sim_stop(sim_data, i + 1));
			min_ttd = (ttd < min_ttd) ? ttd : min_ttd;
			++i;
		}
